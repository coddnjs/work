...
async function renderSelected(){
  ...
  // 기록 표시
  if(data){
    const sSec = parseTime(data.start);
    const eSec = parseTime(data.end);
    const total = eSec - sSec - parseTime(data.break || "0");

    selectedEntry.innerHTML = `
      <div class="entry-card">
        <div class="entry-time" style="font-size:12px;color:#999;">
          ${secToHHMM(sSec)} - ${secToHHMM(eSec)} (${formatHM(total)})
        </div>
...
function calcWeekTotal(selDate){
  ...
  weekTotal.textContent = formatSec(sum);
}
